...
video_dir = os.path.join(relative_path, "videos")
...
frames = []  # To store frames for the entire program
        ...
        while not done_agent or not done_adversary:
            ...
            if rendering:
                ...
                if recording:
                    ...
                    frames.append(frame.swapaxes(0, 1))  # Swap axes for correct orientation
...
def run_neat(config, generations=50, run_last_checkpoint=False):
    ...
    for generation in range(generations):
        p.run(lambda genomes, config: eval_genomes(genomes, config, generation+latest_checkpoint_file), 1)
...
if __name__ == "__main__":
    ...
    if recording:
        ...
        video_filename = os.path.join(video_dir, f"neat_simulation.mp4")
